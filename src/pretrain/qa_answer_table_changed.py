...
class AnswerTable:
    ...
    def __init__(self, dsets=None):
        self.all_ans = json.load(open("/home/myj/code/lxmert_caption/data/lxmert/all_ans.json"))
...
def load_lxmert_qa(path, model, label2ans):
    ...
    ans_weight = answer_state_dict['logit_fc.4.weight']
    ans_bias = answer_state_dict['logit_fc.4.bias']
    import copy
    ...
    print()
    answer_state_dict['logit_fc.4.weight'] = new_answer_weight
    answer_state_dict['logit_fc.4.bias'] = new_answer_bias
    ...
    #         if key not in bert_loaded_keys:  # 只要参数的字段包含fc这个字，就添加到列表中
    #             del_key.append(key)
    #     for key in del_key:
    #         del model.lxrt_encoder.model.state_dict()[key]
    #     model.lxrt_encoder.model.load_state_dict(model.lxrt_encoder.model.state_dict(), strict=False)
    # else:
    #     # Load Bert Weights
    #     bert_model_keys = set(model.lxrt_encoder.model.state_dict().keys())
    #     bert_loaded_keys = set(bert_state_dict.keys())
