...
# #
# #
# args.task_matched= False
# args.task_mask_lm = False
# args.task_obj_predict = False
# args.task_qa = True
#
# args.train = 'vgnococo'  #mscoco_train,mscoco_nominival,vqacp_v2_train
# args.valid = 'mscoco_minival'
#
# args.caption = True
...
# args.epochs = 4
# args.tqdm =True
# args.output = '/home/myj/code/lxmert_caption/snap/pretrain_mulcls'
# args.cap_num = 1
# args.a_cls = 2
# args.con_Loss = False
# args.con_alpha = 0.2
...
def get_tuple(splits: str, bs: int, shuffle=False, drop_last=False, topk=99999999) -> DataTuple:
...
if args.caption:
    class InputFeatures(object):
        ...
        def __init__(self,
                     ...
                     input_ids_caption, input_mask_caption, segment_ids_caption, lm_label_ids_caption,
            ...
            self.input_ids_caption = input_ids_caption
            self.input_mask_caption = input_mask_caption
            self.segment_ids_caption = segment_ids_caption
            self.lm_label_ids_caption = lm_label_ids_caption
...
def convert_example_to_features(example: InputExample, max_seq_length, tokenizer)->InputFeatures:
    ...
    if args.caption:
        caption = tokenizer.tokenize(example.final_cap.strip())
        max_cap_length =args.cap_max_len * args.cap_num
        # Account for [CLS] and [SEP] with "- 2"
        if len(caption) > max_cap_length - 2:
            caption = caption[:(max_cap_length - 2)]

        # Ge random words
        masked_tokens_caption, masked_label_caption = random_word(caption, tokenizer)

        # concatenate lm labels and account for CLS, SEP, SEP
        masked_tokens_caption = ['[CLS]'] + masked_tokens_caption + ['[SEP]']
        input_ids_caption = tokenizer.convert_tokens_to_ids(masked_tokens_caption)

        # Mask & Segment Word
        lm_label_ids_caption = ([-1] + masked_label_caption + [-1])
        input_mask_caption = [1] * len(input_ids_caption)
        if args.caption_sem_type:
            segment_ids_caption = [1] * len(input_ids_caption)
        else:
            segment_ids_caption = [0] * len(input_ids_caption)
        # Zero-pad up to the sequence length.
        while len(input_ids_caption) < max_cap_length:
            input_ids_caption.append(0)
            input_mask_caption.append(0)
            if args.caption_sem_type:
                segment_ids_caption.append(1)
            else:
                segment_ids_caption.append(0)
            lm_label_ids_caption.append(-1)

        assert len(input_ids_caption) == max_cap_length
        assert len(input_mask_caption) == max_cap_length
        assert len(segment_ids_caption) == max_cap_length
        assert len(lm_label_ids_caption) == max_cap_length
    ...
    if args.caption:
        features = InputFeatures(
            ...
            input_mask_caption=input_mask_caption,
            segment_ids_caption=segment_ids_caption,
            lm_label_ids_caption=lm_label_ids_caption,
...
if args.caption:
    LOSSES_NAME = ('con_loss','Mask_LMl', 'Matchedl','Mask_LMv', 'Matchedv',  'Obj', 'Attr', 'Feat', 'QA')
else:
...
if args.caption_model:
    class LXMERT:
        ...
        def forward(self, examples):
            train_features = [convert_example_to_features(example, self.max_seq_length, self.tokenizer) #mask!!
            ...
            if args.caption:
                # language Inputs
                input_ids_caption = torch.tensor([f.input_ids_caption for f in train_features], dtype=torch.long).cuda()
                input_mask_caption = torch.tensor([f.input_mask_caption for f in train_features], dtype=torch.long).cuda()
                segment_ids_caption = torch.tensor([f.segment_ids_caption for f in train_features], dtype=torch.long).cuda()
            ...
            if args.caption:
                # cap Prediction
                lm_labels_caption = torch.tensor([f.lm_label_ids_caption for f in train_features], dtype=torch.long).cuda()
            ...
            loss, losses, ans_logit = self.model(
                ...
                input_ids_caption, segment_ids_caption, input_mask_caption, lm_labels_caption,
        ...
        def train(self, train_tuple: DataTuple, eval_tuple: DataTuple):
            ...
            for epoch in range(args.epochs):
                ...
                for batch in tqdm(train_ld, total=len(train_ld)):
                    ...
                    if (count %1000==0):
                        print("The losses are {}".format(loss))
                        count = 0
                    count += 1
    ...
    class LXMERT:
        ...
        def save(self, name):
            if args.tiny:
                args.output = '/home/myj/code/lxmert_caption/snap/pretrain/'
            torch.save(self.model.state_dict(),
                       os.path.join(args.output, "%s_LXRT.pth" % name))
...
if __name__ == "__main__":
    ...
    valid_batch_size = 2048 if args.multiGPU else args.batch_size
    valid_tuple = get_tuple(args.valid, args.batch_size, shuffle=False, drop_last=False, topk=5000)
