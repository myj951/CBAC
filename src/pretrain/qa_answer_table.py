...
class AnswerTable:
    ...
    def __init__(self, dsets=None):
        ...
        self.all_ans = json.load(open("/home/myj/3.7Tfile/data/lxmert/all_ans.json"))
...
def load_lxmert_qa(path, model, label2ans):
    ...
    # Isolate answer head
    answer_state_dict = {}
    if not args.mul_class:
        for key, value in loaded_state_dict.items():
            if key.startswith("answer_head."):
                answer_state_dict[key.replace('answer_head.', '')] = value
    else:
        for key, value in loaded_state_dict.items():
            if key.startswith("answer_head1."):
                answer_state_dict[key.replace('answer_head1.logit_fc', 'logit_fc1')] = value
            if key.startswith("answer_head2"):
                answer_state_dict[key.replace('answer_head2.logit_fc', 'logit_fc2')] = value

    # Do surgery on answer state dict
    import copy
    if args.mul_class:
        ans_weight1 = answer_state_dict['logit_fc1.3.weight']
        ans_bias1 = answer_state_dict['logit_fc1.3.bias']

        ans_weight2 = answer_state_dict['logit_fc2.3.weight']
        ans_bias2 = answer_state_dict['logit_fc2.3.bias']

        new_answer_weight1 = copy.deepcopy(model_state_dict['logit_fc1.3.weight'])
        new_answer_bias1 = copy.deepcopy(model_state_dict['logit_fc1.3.bias'])

        new_answer_weight2 = copy.deepcopy(model_state_dict['logit_fc2.3.weight'])
        new_answer_bias2 = copy.deepcopy(model_state_dict['logit_fc2.3.bias'])
    else:
        ...
        new_answer_weight = copy.deepcopy(model_state_dict['logit_fc.4.weight'])
        new_answer_bias = copy.deepcopy(model_state_dict['logit_fc.4.bias'])
    ...
    for label, ans in label2ans.items():
        ...
        if answer_table.used(new_ans):
            ...
            if args.mul_class:
                new_answer_weight1[label] = ans_weight1[ans_id_9500]
                new_answer_bias1[label] = ans_bias1[ans_id_9500]
                new_answer_weight2[label] = ans_weight2[ans_id_9500]
                new_answer_bias2[label] = ans_bias2[ans_id_9500]
            else:
            ...
            if args.mul_class:
                new_answer_weight1[label] = 0.
                new_answer_bias1[label] = 0.
                new_answer_weight2[label] = 0.
                new_answer_bias2[label] = 0.
            else:
    ...
    print()
    if args.mul_class:
        answer_state_dict['logit_fc1.3.weight'] = new_answer_weight1
        answer_state_dict['logit_fc1.3.bias'] = new_answer_bias1

        answer_state_dict['logit_fc2.3.weight'] = new_answer_weight2
        answer_state_dict['logit_fc2.3.bias'] = new_answer_bias2
    else:
    ...
    print(ans_loaded_keys - model_keys)
    ...
    if args.caption:
        answer_state_dict1 ={}
        for key, value in answer_state_dict.items():
            # if key in ['logit_fc.2.weight', 'logit_fc.2.bias']:
            #     print(key)
            # else:
            #     answer_state_dict1[key] = value

            if key in ['logit_fc.3.weight', 'logit_fc.3.bias']:
                print(key)
            else:
                answer_state_dict1[key] = value
        answer_state_dict = answer_state_dict1
        ans_loaded_keys = set(answer_state_dict.keys())
        print(ans_loaded_keys - model_keys)
